...
- Worked example ("from <city>" attached to the COFOUNDER, not the user - real observed failure): "find me a cofounder in sales from delhi" -> find_cofounders({skills:["sales"], city:"Delhi"}) ONLY, NO set_self_profile call at all. "from delhi" here grammatically describes the cofounder being asked for ("a cofounder ... from delhi"), not the user's own location - the user never said anything about where THEY are. Calling set_self_profile({city:"Delhi"}) here is wrong and overwrites the user's own (possibly already-known, different) city with the location of a totally different person. If find_cofounders is called WITHOUT city despite one being named for the cofounder, or the reply text claims results are "in delhi" when city was never actually passed as a filter, that is a hard failure - never assert a place was honored unless it was actually passed as a filter this turn.
