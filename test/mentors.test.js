...
const {
  ...
  filterByArea,
  matchByExpertise,
} = require('../src/domain/mentors');
const { MENTORS } = require('../src/domain/mentors.data');
const { AREA_KEYS } = require('../src/domain/mentorAreas');
const fmt = require('../src/bot/format');

test('every taxonomy area has at least one active mentor', () => {
  const areas = areasWithCounts(MENTORS);
  const keysWithMentors = areas.map((a) => a.key);
  for (const key of AREA_KEYS) {
    assert.ok(keysWithMentors.includes(key), `area "${key}" has no mentor`);
  }
});
...
test('filterByArea returns only mentors tagged with that area, ordered', () => {
  const fr = filterByArea(MENTORS, 'fundraising');
...
test('filterByArea on an unknown area is empty', () => {
  assert.strictEqual(filterByArea(MENTORS, 'nonexistent').length, 0);
});

test('matchByExpertise ranks topical mentors and ignores short tokens', () => {
  const fundraising = matchByExpertise(MENTORS, 'fundraising').map((s) => s.slug);
  ...
  assert.strictEqual(matchByExpertise(MENTORS, 'xyzzy').length, 0);
  // sub-3-char query yields nothing
  assert.strictEqual(matchByExpertise(MENTORS, 'go').length, 0);
});

test('matchByExpertise resolves an explicit booking by name (top-ranked)', () => {
  const byName = matchByExpertise(MENTORS, 'book varun chawla');
  assert.strictEqual(byName[0].slug, 'varun-chawla'); // name hit outranks topic hits
  assert.ok(matchByExpertise(MENTORS, 'ashmita').some((s) => s.slug === 'ashmita-dutta'));
});

test('mentorRow / areaRow produce valid WhatsApp list rows', () => {
  const row = fmt.mentorRow(MENTORS[0]);
  assert.strictEqual(row.id, `mentor:${MENTORS[0].slug}`);
  assert.strictEqual(row.title, MENTORS[0].name);
...
test('mentorCard includes LinkedIn only when present', () => {
  const withLi = fmt.mentorCard(MENTORS.find((s) => s.linkedin_url));
  assert.match(withLi, /linkedin\.com/);
  const withoutLi = fmt.mentorCard(MENTORS.find((s) => !s.linkedin_url));
...
test('bookingMessage carries the external link plus both guardrails', () => {
  const msg = fmt.bookingMessage(MENTORS[0]);
  assert.ok(msg.includes(MENTORS[0].booking_url));
  assert.match(msg, /prep doc/i);
  assert.match(msg, /feedback form/i);
});

test('every mentor has a slug, booking url, and at least one valid area', () => {
  const slugs = new Set();
